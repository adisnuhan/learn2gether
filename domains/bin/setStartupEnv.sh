...
if [ "${STARTUP_GROUP}" = "AdminServerStartupGroup" ] ; then
	...
	if [ "${POST_CLASSPATH}" != "" ] ; then
		POST_CLASSPATH="${POST_CLASSPATH}${CLASSPATHSEP}${MW_HOME}/oracle_common/modules/com.oracle.cie.config-wls-online_8.1.0.0.jar"
		export POST_CLASSPATH
	else
		POST_CLASSPATH="${MW_HOME}/oracle_common/modules/com.oracle.cie.config-wls-online_8.1.0.0.jar"
		export POST_CLASSPATH
	fi
fi
